...
function activate(context) {
    console.log('REC Language extension is now active!');
    // Register a diagnostic collection for REC files
...
origins: ["http://localhost:3000", "https://api.example.com"]`, 'rec');
    ...
    const completionProvider = vscode.languages.registerCompletionItemProvider('rec', {
        provideCompletionItems(document, position, token, context) {
            ...
            // Suggest REC keywords at the beginning of lines
            ...
            // Suggest include at the beginning
            if (linePrefix.trim() === '' || linePrefix.trim() === '#') {
            ...
            // Suggest built-in types after colon
        ...
        provideDocumentSymbols(document, token) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (enumMatch) {
                    ...
                    const symbol = new vscode.DocumentSymbol(name, 'enum', vscode.SymbolKind.Enum, new vscode.Range(startPos, endPos), new vscode.Range(startPos, endPos));
                    symbols.push(symbol);
                }
                ...
                if (typeMatch) {
                    ...
                    const endPos = new vscode.Position(endLine, lines[endLine].length);
                    const symbol = new vscode.DocumentSymbol(name, 'type', vscode.SymbolKind.Struct, new vscode.Range(startPos, endPos), new vscode.Range(startPos, endPos));
                    symbols.push(symbol);
                }
                // Find top-level keys in the root object
                ...
                if (keyMatch && !line.includes('@') && !line.includes('#')) {
                    ...
                    const symbol = new vscode.DocumentSymbol(name, 'property', vscode.SymbolKind.Property, new vscode.Range(startPos, endPos), new vscode.Range(startPos, endPos));
                    symbols.push(symbol);
                }
            }
            return symbols;
    ...
    const formattingProvider = vscode.languages.registerDocumentFormattingEditProvider('rec', {
        provideDocumentFormattingEdits(document, options, token) {
            // This is a simple formatter - in production you'd use the REC parser
            const edits = [];
            const text = document.getText();
            const lines = text.split('\n');
            let indentLevel = 0;
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (newLine !== lines[i]) {
                    edits.push(vscode.TextEdit.replace(new vscode.Range(i, 0, i, lines[i].length), newLine));
                }
    ...
    // Register command to convert REC to JSON
    const convertToJsonCommand = vscode.commands.registerCommand('rec.convertToJson', () => {
        ...
        // In a real implementation, you would call the REC CLI here
        vscode.window.showInformationMessage('REC to JSON conversion would happen here');
    });
    context.subscriptions.push(convertToJsonCommand);
}
exports.activate = activate;
function validateRecFile(document, diagnosticCollection) {
    const diagnostics = [];
    const text = document.getText();
    const lines = text.split('\n');
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Check for specific REC function syntax errors
        // Only check for quotes in the actual function calls
        const socketMatch = line.match(/socket\s*\(\s*([^)]+)\s*\)/);
        if (socketMatch && !socketMatch[1].includes('"')) {
            const startIndex = line.indexOf('socket(');
            const diagnostic = new vscode.Diagnostic(new vscode.Range(i, startIndex, i, startIndex + socketMatch[0].length), 'Socket addresses must be quoted strings: socket("127.0.0.1:8080")', vscode.DiagnosticSeverity.Error);
            diagnostics.push(diagnostic);
        }
        const urlMatch = line.match(/url\s*\(\s*([^)]+)\s*\)/);
        if (urlMatch && !urlMatch[1].includes('"')) {
            const startIndex = line.indexOf('url(');
            const diagnostic = new vscode.Diagnostic(new vscode.Range(i, startIndex, i, startIndex + urlMatch[0].length), 'URLs must be quoted strings: url("https://example.com")', vscode.DiagnosticSeverity.Error);
            diagnostics.push(diagnostic);
        }
        const pubkeyMatch = line.match(/pubkey\s*\(\s*([^)]+)\s*\)/);
        if (pubkeyMatch && !pubkeyMatch[1].includes('"')) {
            const startIndex = line.indexOf('pubkey(');
            const diagnostic = new vscode.Diagnostic(new vscode.Range(i, startIndex, i, startIndex + pubkeyMatch[0].length), 'Public keys must be quoted strings: pubkey("base58key...")', vscode.DiagnosticSeverity.Error);
            diagnostics.push(diagnostic);
        }
    ...
    // Only report brace mismatch at the end of the document
    if (braceBalance !== 0) {
        const lastLine = lines.length - 1;
        const diagnostic = new vscode.Diagnostic(new vscode.Range(lastLine, 0, lastLine, lines[lastLine].length), `Unmatched braces in document: ${braceBalance > 0 ? braceBalance + ' unclosed {' : Math.abs(braceBalance) + ' extra }'}`, vscode.DiagnosticSeverity.Error);
        diagnostics.push(diagnostic);
    }
